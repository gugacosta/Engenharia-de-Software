...
## 3. Responsive and Acessibility
  - Why to be acessible?
    - Acessibility can be defined as: qualities that make an experience open to all, even the system that was developed for operation in background needs to be accesible for administrator or developer, develope a software able to have your all functions acessible for all users is a big challange because exist a lot kind of disability, then two important questions should be answered before develope an interface: 
    1. What's is the necessity of acessibility most of your user?
    2. What the cost of add more acessible features?

  - Some tips to develope a good acessible interface:
    1. Use good contrast and letter size
    2. Make linear logical layout
    3. Describe images and vídeos
    4. Make hot keys to access all features
    5. Use simple colours
    6. Make big hiperlink area
    7. Add audio and video materials

  - What's is responsive?
    - Responsive can be defined: A aprooach that creates dynamic changes to the appearance of a system interfaces, depending on the screen size and orietation of device being used to view it, a responsive software can be used by many devices with the same user experience.

  - How to project a interface for multitude of devices?
   1. In the world was increased the use of smartphone than desktops and as the display of it is smaller was developed the aproach "Mobile First" because in the most of interfaces is more easy to maximise the display size than minimise.
   2. Fluid: The conection of the user with the interface needs to be soft and continous, then the topics should be conected and with few comands the user must be able to acess all content, one way to make it is organize the layout with moving columns.  
   3. Tiny Tweaks: For minimalist interface make few changes.
   4. Off Canvas: Group features in columns off the display but that can be acessible with a few comands.
   5. Performance: Think that the mobile network can be more slow than desktop, then try use light resources.
